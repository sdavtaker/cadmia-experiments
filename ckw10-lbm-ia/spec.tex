...
\begin{enumerate}
  ...
  \item \textbf{UA-DEVS $\to$ IA-DEVS.}
        Each UA-DEVS uncertain set is enclosed by an interval.
        The set-to-interval mapping for each LBM component is:
        \begin{itemize}
          \item LG and servers: the UA-DEVS set $(0,+\infty)$ is
            enclosed by the IA-DEVS interval $(0,+\infty)$,
            the exact support of the exponential distribution.
          \item WB: the UA-DEVS set $\{1,2\}$ is enclosed by the
            IA-DEVS interval $[1,2]$.  The enclosure is exact for
            integer outputs since no integers lie strictly between
            $1$ and $2$.
        \end{itemize}
\end{enumerate}
...
\begin{align*}
  ...
  \Lambda^S(\tilde s)
    &= [\mathtt{task\_completed},\,\mathtt{task\_completed}]
\end{align*}
...
% ─────────────────────────────────────────────────────────────────────────────
\subsection*{BFS Simulation Complexity}

A full IA-DEVS BFS simulation of the LBM at operational scale
($d_r = 10$, $T = 10\,000$\,s, ${\sim}10^5$ events) requires exponential
resources: whenever the LG time-advance interval $(0,+\infty)$ and the
...
With $(0,+\infty)$ intervals the two ranges always overlap, so every event
produces a branch, giving $2^{{\sim}10^5}$ trajectories at operational scale.
